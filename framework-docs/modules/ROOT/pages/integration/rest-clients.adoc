...
For testing, an HTTP Interface Client can also be backed by `WebTestClient` or
`RestTestClient` so that an `@HttpExchange` interface can be invoked against a mock
server (for example a controller bound via `WebTestClient.bindToController(...)` or
