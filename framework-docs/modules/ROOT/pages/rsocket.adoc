...
<1> Use `PathPatternRouteMatcher`, if `spring-web` is present, for efficient
    route matching.
<2> Create a responder from a class with `@MessageMapping`/`@RSocketExchange`
and/or `@ConnectMapping` methods.
...
In xref:rsocket.adoc#rsocket-annot-responders[Annotated Responders], `@ConnectMapping`,
`@MessageMapping` and `@RSocketExchange` methods support an
...
RSocket responders can be implemented as `@MessageMapping` or `@RSocketExchange` methods,
and `@ConnectMapping` methods.
`@MessageMapping` and `@RSocketExchange` methods handle individual requests while `@ConnectMapping` methods handle
...
configuration to detect `@Controller` beans with `@MessageMapping`,
`@RSocketExchange` and `@ConnectMapping` methods:
...
[[rsocket-annot-messagemapping-rsocketexchange]]
=== `@MessageMapping` and `@RSocketExchange`

Once xref:rsocket.adoc#rsocket-annot-responders-server[server] or
xref:rsocket.adoc#rsocket-annot-responders-client[client] responder configuration is in place, annotated server-side methods can be used as follows:
...
The above annotated methods respond to a Request-Stream interaction having the
route "locate.radars.within".


They support flexible method signatures with the option to
...
The RSocket interaction type that an annotated method supports is determined from
...
NOTE: The `@MessageMapping` and `@RSocketExchange` annotations for server
responders can be used nearly interchangeably, with the exception that
while `@MessageMapping` accepts a `String` array as its `value` parameter,
only a single `String` can be passed as the `value` of `@RSocketExchange`.

TIP: Both `@MessageMapping` and `@RSocketExchange` annotations can also be used at
class level to specify a common prefix for all the method routes.
...
xref:rsocket.adoc#rsocket-annot-messagemapping-rsocketexchange[@MessageMapping or @RSocketExchange]
but based on metadata and data from the `SETUP` and
