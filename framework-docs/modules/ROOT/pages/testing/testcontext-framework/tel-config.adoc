...
* `AotCacheTestExecutionListener`: Coordinates JVM AOT cache recording during integration
  tests. It activates automatically when the test JVM is started with AOT cache recording
  flags (see xref:integration/aot-cache.adoc#aot-cache-integration-tests[Generating the cache from integration tests]).
