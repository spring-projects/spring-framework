...
[NOTE]
====
When combining `@Retryable` with other proxy-based features (e.g., `@Transactional` or AOP advice), ensure that the order of proxies is correct.
Improper ordering may result in retries not being applied as expected.
====
