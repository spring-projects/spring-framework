 ...
 * @see AliasFors
 * @see AnnotatedElementUtils#getMergedAnnotationWithMultipleAliases(java.lang.reflect.AnnotatedElement, java.lang.Class)
 * @see MergedAnnotations#fromMultipleAliasesAnnotations(java.lang.reflect.AnnotatedElement, org.springframework.core.annotation.MergedAnnotations.SearchStrategy, org.springframework.core.annotation.RepeatableContainers)
 * @since 5.3
 */
final class MultipleAliasAnnotations implements MergedAnnotations {

	/**
	 * Shared instance that can be used when there are no annotations.
	 */
	static final MultipleAliasAnnotations NONE = new MultipleAliasAnnotations(
	...
	@Nullable
	private volatile List<MultipleAliasAnnotations.Aggregate> aggregates;

	private MultipleAliasAnnotations(AnnotatedElement element, MergedAnnotations.SearchStrategy searchStrategy,
	...
	private MultipleAliasAnnotations(@Nullable Object source, Annotation[] annotations,
									 RepeatableContainers repeatableContainers, AnnotationFilter annotationFilter) {
		this.source = source;
		this.element = null;
	...
	public <A extends Annotation> boolean isPresent(Class<A> annotationType) {
		...
		return Boolean.TRUE.equals(scan(annotationType,
				MultipleAliasAnnotations.IsPresent.get(this.repeatableContainers, this.annotationFilter, false)));
	}
	...
	public <A extends Annotation> boolean isDirectlyPresent(Class<A> annotationType) {
		...
		return Boolean.TRUE.equals(scan(annotationType,
				MultipleAliasAnnotations.IsPresent.get(this.repeatableContainers, this.annotationFilter, true)));
	}
	...
	private <A extends Annotation> Spliterator<MergedAnnotation<A>> spliterator(@Nullable Object annotationType) {
		return new MultipleAliasAnnotations.AggregatesSpliterator<>(annotationType, getAggregates());
	}

	private List<MultipleAliasAnnotations.Aggregate> getAggregates() {
		List<MultipleAliasAnnotations.Aggregate> aggregates = this.aggregates;
		if (aggregates == null) {
			aggregates = scan(this, new MultipleAliasAnnotations.AggregatesCollector());
	...
	static MergedAnnotations from(AnnotatedElement element, MergedAnnotations.SearchStrategy searchStrategy,
		...
		return new MultipleAliasAnnotations(element, searchStrategy, repeatableContainers, annotationFilter);
	}
	...
	static MergedAnnotations from(@Nullable Object source, Annotation[] annotations,
		...
		return new MultipleAliasAnnotations(source, annotations, repeatableContainers, annotationFilter);
	}
	...
	private static final class IsPresent implements AnnotationsProcessor<Object, Boolean> {
		 ...
		 */
		private static final MultipleAliasAnnotations.IsPresent[] SHARED;

		static {
			SHARED = new MultipleAliasAnnotations.IsPresent[4];
			SHARED[0] = new MultipleAliasAnnotations.IsPresent(RepeatableContainers.none(), AnnotationFilter.PLAIN, true);
			SHARED[1] = new MultipleAliasAnnotations.IsPresent(RepeatableContainers.none(), AnnotationFilter.PLAIN, false);
			SHARED[2] = new MultipleAliasAnnotations.IsPresent(RepeatableContainers.standardRepeatables(), AnnotationFilter.PLAIN, true);
			SHARED[3] = new MultipleAliasAnnotations.IsPresent(RepeatableContainers.standardRepeatables(), AnnotationFilter.PLAIN, false);
		}
		...
		static MultipleAliasAnnotations.IsPresent get(RepeatableContainers repeatableContainers,
													  AnnotationFilter annotationFilter, boolean directOnly) {
			...
			return new MultipleAliasAnnotations.IsPresent(repeatableContainers, annotationFilter, directOnly);
		}
	}
	...
	private class MergedMultipleAliasAnnotationFinder<A extends Annotation>
		...
		private MergedAnnotation<A> process(
			...
			AnnotationTypeMappings mappings = AnnotationTypeMappings.forMultipleAnnotationType(
	...
	/**
	 * {@link AnnotationsProcessor} that collects {@link MultipleAliasAnnotations.Aggregate} instances.
	 */
	private class AggregatesCollector implements AnnotationsProcessor<Object, List<MultipleAliasAnnotations.Aggregate>> {

		private final List<MultipleAliasAnnotations.Aggregate> aggregates = new ArrayList<>();

		@Override
		@Nullable
		public List<MultipleAliasAnnotations.Aggregate> doWithAnnotations(Object criteria, int aggregateIndex,
																		  @Nullable Object source, Annotation[] annotations) {
			this.aggregates.add(createAggregate(aggregateIndex, source, annotations));
			return null;
		}

		private MultipleAliasAnnotations.Aggregate createAggregate(int aggregateIndex, @Nullable Object source, Annotation[] annotations) {
			List<Annotation> aggregateAnnotations = getAggregateAnnotations(annotations);
			return new MultipleAliasAnnotations.Aggregate(aggregateIndex, source, aggregateAnnotations);
		}
		...
		@Override
		public List<MultipleAliasAnnotations.Aggregate> finish(@Nullable List<MultipleAliasAnnotations.Aggregate> processResult) {
	...
	private class AggregatesSpliterator<A extends Annotation> implements Spliterator<MergedAnnotation<A>> {
		...
		private final List<MultipleAliasAnnotations.Aggregate> aggregates;
		...
		AggregatesSpliterator(@Nullable Object requiredType, List<MultipleAliasAnnotations.Aggregate> aggregates) {
			this.requiredType = requiredType;
			this.aggregates = aggregates;
		...
		public boolean tryAdvance(Consumer<? super MergedAnnotation<A>> action) {
			while (this.aggregateCursor < this.aggregates.size()) {
				MultipleAliasAnnotations.Aggregate aggregate = this.aggregates.get(this.aggregateCursor);
		...
		private boolean tryAdvance(MultipleAliasAnnotations.Aggregate aggregate, Consumer<? super MergedAnnotation<A>> action) {
		...
		@Nullable
		private AnnotationTypeMapping getNextSuitableMapping(MultipleAliasAnnotations.Aggregate aggregate, int annotationIndex) {
		...
		public long estimateSize() {
			...
			for (int aggregateIndex = this.aggregateCursor;
				...
				MultipleAliasAnnotations.Aggregate aggregate = this.aggregates.get(aggregateIndex);
				for (int annotationIndex = 0; annotationIndex < aggregate.size(); annotationIndex++) {
