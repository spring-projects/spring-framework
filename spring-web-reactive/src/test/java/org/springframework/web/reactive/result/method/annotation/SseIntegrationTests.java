...
import org.springframework.http.codec.ServerSideEvent;
...
public class SseIntegrationTests extends AbstractHttpHandlerIntegrationTests {
	...
	static class SseController {
		...
		Flux<ServerSideEvent<String>> sse() {
			return Flux.interval(Duration.ofMillis(100)).map(l -> ServerSideEvent.<String>builder().data("foo")
                    .id(Long.toString(l))
                    .comment("bar")
                    .build()).take(2);
		}
