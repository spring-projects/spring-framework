...
public abstract class MultiReleaseExtension {
  ...
  private void releaseVersions(
      String mainSourceDirectory, String testSourceDirectory, int... javaVersions) {
  ...
  private void addLanguageVersion(
      int javaVersion, String mainSourceDirectory, String testSourceDirectory) {
    ...
    SourceSet testSourceSet =
        sourceSets.create(
            javaN + "Test", srcSet -> srcSet.getJava().srcDir(testSourceDirectory + javaN));
    ...
    FileCollection mainClasses =
        objects
            .fileCollection()
            .from(
                sourceSets.getByName(SourceSet.MAIN_SOURCE_SET_NAME).getOutput().getClassesDirs());
    dependencies.add(javaN + "Implementation", mainClasses);

    tasks.named(
        langSourceSet.getCompileJavaTaskName(),
        JavaCompile.class,
        task -> task.getOptions().getRelease().set(javaVersion));
    tasks.named(
        testSourceSet.getCompileJavaTaskName(),
        JavaCompile.class,
        task -> task.getOptions().getRelease().set(javaVersion));

    TaskProvider<Test> testTask =
        createTestTask(
            javaVersion, testSourceSet, sharedTestSourceSet, langSourceSet, sharedSourceSet);
    tasks.named("check", task -> task.dependsOn(testTask));

    configureMultiReleaseJar(javaVersion, langSourceSet);
  }

  private TaskProvider<Test> createTestTask(
      int javaVersion,
      SourceSet testSourceSet,
    ...
    testCompileOnly.extendsFrom(
        configurations.getByName(sharedTestSourceSet.getCompileOnlyConfigurationName()));
    testCompileOnly
        .getDependencies()
