 ...
 * of operand evaluation. For example in 'list[index++]++' without a value reference for 'list[index++]'
 * it would be necessary to evaluate list[index++] twice (once to get the value, once to determine
 * where the value goes) and that would double increment index.
 * 
 * @author Andy Clement
 * @since 3.2
 */
public interface ValueRef {
	...
	static class NullValueRef implements ValueRef {
		...
		public void setValue(Object newValue) {	
			// The exception position '0' isn't right but the overhead of creating instances of this per node (where
			// the node is solely for error reporting) would be unfortunate.
	...
	static class TypedValueHolderValueRef implements ValueRef {
		...
		public void setValue(Object newValue) {
			throw new SpelEvaluationException(node.pos,SpelMessage.NOT_ASSIGNABLE,node.toStringAST());
		}
