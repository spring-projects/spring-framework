...
/**
 * The bean factory for EasyMock mocks. This bean factory helps to define mock
 * beans in Spring context which are bound to a given EasyMock control. In Unit
 * Test these mocks can be referred by type and all operations (reset / replay /
 * verify) for all mocks can be performed via one control. Assumes that Spring
 * context has a bean with name "mocksControl" which will be used to create
 * mocks (this is customizable via {@link #setMocksControl()}) or otherwise
 * factory will fallback to creating a separate control per mock. Example of
 * usage:
 * 
 * <pre>
 ...
 * 	private Dao dao;
 * 
 * 	// @Autowired will also perfectly work here (if no ambiguity in context):
 * 	&#064;Resource
...
public class EasyMockFactoryBean<T> implements FactoryBean<T>,
	...
	/**
	 * Sets {@link IMocksControl} on this factory. This is necessary when you
	 * want to share the same {@link IMocksControl} among all mocks.
	 */
	...
	/**
	 * The same as {@link #setMockClass(Class)} but provided for better
	 * expressibility.
	 */
	...
	public void afterPropertiesSet() throws FatalBeanException {
		...
		if (mocksControl == null) {
			...
			mocksControl = beanFactory.getBean(DEFAULT_MOCKS_CONTROL_BEAN_NAME,
					IMocksControl.class);
