...
public class BeanDefinitionParserDelegate {
	...
	public void parseMetaElements(Element ele, BeanMetadataAttributeAccessor attributeAccessor) {
		...
		for (int i = 0; i < nl.getLength(); i++) {
			Node node = nl.item(i);
			// 标签名为 meta
			// 如：<meta key="special-data" value="sprecial stragey" />
	 ...
	 * </bean>
	 *
	 * 配置文件中使用 constructor-arg 元素对其配置，该元素可以实现对 StudentService 自动寻找对应的构造函数，并在初始化的时候将值当做参数进行设置。
	 *
	 *
	 */
	...
	/**
	 * 对于 constructor-arg 子元素的嵌套子元素 处理
	 */
	@Nullable
	...
	/**
	 * 调用 #getNamespaceURI((Node node) 方法，获取 namespaceUri
	 * 调用 XmlReaderContext#getNamespaceHandlerResolver() 方法，获得命名空间的解析器
	 * 调用 NamespaceHandlerResolver#resolve(String namespaceUri) 方法，根据 namespaceUri 获取相应的 Handler 对象。这个映射关系我们在 spring.handlers 中已经定义了，所以只需要找到该类，然后初始化返回
	 * 调用 NamespaceHandler#parse(Element element, ParserContext parserContext) 方法，调用自定义的 Handler 处理
	 */
	@Nullable
	...
	public BeanDefinitionHolder decorateIfRequired(
		...
		// <2> 过滤掉默认命名标签 不是默认的命名空间，则根据该命名空间获取相应的处理器。
