...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	protected Object doCreateBean(final String beanName, final RootBeanDefinition mbd, final @Nullable Object[] args)
		...
		//封装被创建的Bean对象 （这个BeanWrapper是用来持有创建出来的bean对象的）
		...
		//如果是单例、运行提前暴露 bean、当前 bean 正在创建中
		...
		if (earlySingletonExposure) {
			...
			/*
			 * 这个三级缓存才是解决 Spring Bean 循环依赖的诀窍所在。
		...
		//这里是对bean的初始化，依赖注入往往在这里发生，这个exposedObject在初始化处理完以后会返回组委依赖注入完成后的bean
		Object exposedObject = bean;
		try {
			//将Bean实例对象封装，并且Bean定义中配置的属性值赋值给实例对象
			populateBean(beanName, mbd, instanceWrapper);

			//初始化Bean对象,包括设置Aware等
		...
		if (earlySingletonExposure) {
			...
			/* 33333333333333333->
			 * 这里再次调用getSingleton 从一级缓存二级缓存中获取
			 * 因为前面调用了addSingletonFactory方法为bean添加单例工厂
			 * 所以getSingleton在一级二级缓存中没有获取到会使用单例工厂创建bean的实例
			 */
	...
	protected BeanWrapper instantiateBean(final String beanName, final RootBeanDefinition mbd) {
		try {
			...
			if (System.getSecurityManager() != null) {
				...
				//CglibSubclassingInstantiationStrategy#SimpleInstantiationStrategy#instantiate
	...
	protected void applyPropertyValues(String beanName, BeanDefinition mbd, BeanWrapper bw, PropertyValues pvs) {
		...
		try {
			//AbstractPropertyAccessor#setPropertyValues  AbstractNestablePropertyAccessor#setPropertyValue
	...
	protected Object initializeBean(final String beanName, final Object bean, @Nullable RootBeanDefinition mbd) {
		if (System.getSecurityManager() != null) {
			...
			//调用aware方法
			//此处只处理了 BeanNameAware、BeanClassLoaderAware、BeanFactoryAware
			invokeAwareMethods(beanName, bean);
		}
