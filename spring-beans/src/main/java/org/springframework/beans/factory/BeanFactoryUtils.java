 ...
 * <p>
 * beanFactory工具类(BeanFactoryUtils)
 * <p>在bean工厂运营的便利方法, 特别是在{@link ListableBeanFactory}界面上.
 * <p>返回bean计数、bean名称或bean实例, 考虑豆工厂的嵌套层次结构
 * (ListableBeanFactory接口上定义的方法没有, 与BeanFactory接口上定义的方法相比)。
 *
 * @author Rod Johnson
...
public abstract class BeanFactoryUtils {
	 ...
	 * of each FactoryBean).
	 *
	 * <p>包含祖先类型的bean名称(beanNamesForTypeIncludingAncestors)
	 * <p>获取给定类型的所有bean名称, 包括在祖先工厂中定义的名称。在重写bean定义的情况下, 将返回唯一的名称
	 * <p> 如果设置了“allowEagerInit”标志, 则考虑由FactoryBean创建的对象, 这意味着FactoryBean将被初始化。
	 * 如果FactoryBean创建的对象不匹配, 则原始FactoryBean本身将与类型匹配。
	 * 如果未设置“allowEagerInit”, 则只检查原始FactoryBean（不需要初始化每个FactoryBean）
	 *
