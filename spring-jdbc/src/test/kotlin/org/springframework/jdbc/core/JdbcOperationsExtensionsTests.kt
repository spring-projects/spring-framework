...
class JdbcOperationsExtensionsTests {
	...
	fun `queryForObject with argTypes`() {
		val i = template.queryForObject("select age from customer where id = ?", arrayOf(3), intArrayOf(JDBCType.INTEGER.vendorTypeNumber), Int::class)
		assertEquals(22, i)
	}

	@Test
	fun `queryForObject with reified type and argTypes`() {
		val i: Int = template.queryForObject("select age from customer where id = ?", arrayOf(3), intArrayOf(JDBCType.INTEGER.vendorTypeNumber))
		assertEquals(22, i)
	}
	...
	fun `queryForList with argTypes`() {
		val i = template.queryForList(sql = "select age from customer where id = ?", args = arrayOf(3), argTypes = intArrayOf(JDBCType.INTEGER.vendorTypeNumber), elementType = Int::class)
		assertEquals(22, i.first())
	}
