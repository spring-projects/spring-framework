...
final class MultiRowReader extends Reader {
    ...
    void addRow(String row) {
        ...
        checkArgument(!closed, "Precondition violated: This reader has not been closed.");
    ...
    public int read(char[] destination, int offset, int length) {
        checkArgument(length > 0, "Precondition violated: length > 0.");

        if (rows.isEmpty()) {
            checkArgument(closed, "Precondition violated: This reader has been closed. " +
