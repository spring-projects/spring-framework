...
/**
 * Encoder that supports a stream of {@link ServerSideEvent}s and also plain
 * {@link Object}s which is the same as an {@link ServerSideEvent} with data
 * only.
 *
 ...
 * @since 5.0
 */
public class ServerSideEventHttpMessageWriter implements HttpMessageWriter<Object> {
    ...
    public ServerSideEventHttpMessageWriter() {
        this.dataEncoders = Collections.emptyList();
    }

    public ServerSideEventHttpMessageWriter(List<Encoder<?>> dataEncoders) {
    ...
    private ServerSideEvent<?> toSseEvent(Object data, ResolvableType type) {
	    return ServerSideEvent.class.isAssignableFrom(type.getRawClass())
			    ? (ServerSideEvent<?>) data
			    : ServerSideEvent.builder().data(data).build();
    }
