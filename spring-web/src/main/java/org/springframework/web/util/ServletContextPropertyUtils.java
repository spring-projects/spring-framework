 ...
 *
 * <p>A text may contain {@code ${...}} placeholders, to be resolved as servlet context init parameters or
 * system properties: e.g. {@code ${user.dir}}.  Default values can be supplied using the ":" separator between key
 * and value.
 *
...
public abstract class ServletContextPropertyUtils {
	...
	/**
	 * Resolve ${...} placeholders in the given text, replacing them with corresponding servlet context init parameter or
     * system property values. Unresolvable placeholders with no default value are ignored and passed through unchanged
	...
	public static String resolvePlaceholders(String text, ServletContext servletContext, boolean ignoreUnresolvablePlaceholders) {
		...
		String value = helper.replacePlaceholders(text, new ServletContextPlaceholderResolver(text, servletContext));
		if (value == null) {
			value = SystemPropertyUtils.resolvePlaceholders(text, ignoreUnresolvablePlaceholders);
		}
		return value;
	}
	...
	private static class ServletContextPlaceholderResolver implements PropertyPlaceholderHelper.PlaceholderResolver {
        ...
        public String resolvePlaceholder(String placeholderName) {
            try {
                return servletContext.getInitParameter(placeholderName);
            }
            catch (Throwable ex) {
                System.err.println("Could not resolve placeholder '" + placeholderName + "' in [" +
                        this.text + "] as a servletcontext init-parameter: " + ex);
                return null;
            }
        }
    }
