 ...
 *
 * <p>This context will create, initialize and run an {@link EmbeddedServletContainer}
 * by searching for a single {@link EmbeddedServletContainerFactory} bean within the
 * {@link ApplicationContext} itself. The {@link EmbeddedServletContainerFactory} is
 * free to use standard Spring concepts (such as dependency injection, lifecycle
 * callbacks and property placeholder variables).
 *
 * <p>In addition, any {@link Servlet} or {@link Filter} beans defined in the context will
 * be automatically registered with the embedded Servlet container. In the case of a
 * single Servlet bean, the '/*' mapping will be used. If multiple Servlet beans are
 * found then the lowercase bean name will be used as a mapping prefix. Filter beans will
 * be mapped to all URLs ('/*').
 *
 * <p>For more advanced configuration, the context can instead define beans that implement
 * the {@link ServletContextInitializer} interface (most often
 * {@link ServletRegistrationBean}s and/or {@link FilterRegistrationBean}s). To prevent
 * double registration, the use of {@link ServletContextInitializer} beans will disable
 * automatic Servlet and Filter bean registration.
 *
 * <p>Although this context can be used directly, most developers should consider using
 * the {@link AnnotationConfigEmbeddedWebApplicationContext} or
...
public class EmbeddedWebApplicationContext extends GenericWebApplicationContext {
	 ...
	 * the embedded servlet container. By default this method searches for a suitable
	 ...
	 * {@link ServletContextInitializer} beans, if none are found it will instead
	 * search for {@link Servlet} and {@link Filter} beans.
	 */
	protected Collection<ServletContextInitializer> getServletContextInitializerBeans() {
		...
		if(initializers.isEmpty()) {
			...
			for (Entry<String, Servlet> servletBean : servletBeans) {
				String url = (servletBeans.size() == 1 ? "/*" : "/" + servletBean.getKey().toLowerCase() + "/*");
				ServletRegistrationBean registration = new ServletRegistrationBean(servletBean.getValue(), url);
			...
			for (Entry<String, Filter> filterBean : getOrderedBeansOfType(Filter.class)) {
				FilterRegistrationBean registration = new FilterRegistrationBean(filterBean.getValue());
	 ...
	 * {@link ServletContextInitializer#onStartup(ServletContext)} and is similar
	 * to the functionality usually provided by a {@link ContextLoaderListener}.
	...
	private <T> SortedSet<Entry<String, T>> getOrderedBeansOfType(Class<T> type) {
		SortedSet<Entry<String, T>> beans = new TreeSet<Entry<String, T>>(new Comparator<Entry<String, T>>() {
			@Override
