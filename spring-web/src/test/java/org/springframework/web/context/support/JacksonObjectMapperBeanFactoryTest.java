...
public class JacksonObjectMapperBeanFactoryTest {
	...
	public void testCommonSetters() {
		...
		assertFalse(objectMapper.getSerializationConfig().isEnabled(SerializationConfig.Feature.AUTO_DETECT_FIELDS));
		assertFalse(objectMapper.getDeserializationConfig().isEnabled(DeserializationConfig.Feature.AUTO_DETECT_FIELDS));

		assertFalse(objectMapper.getSerializationConfig().isEnabled(SerializationConfig.Feature.AUTO_DETECT_GETTERS));
		assertFalse(objectMapper.getDeserializationConfig()
				.isEnabled(DeserializationConfig.Feature.AUTO_DETECT_SETTERS));

		assertFalse(objectMapper.getSerializationConfig().isEnabled(SerializationConfig.Feature.FAIL_ON_EMPTY_BEANS));

		assertTrue(objectMapper.getSerializationConfig().isEnabled(SerializationConfig.Feature.INDENT_OUTPUT));
	}
	...
	public void testCompleteFlow() {
		...
		beanFactory.setFeaturesToEnable(new Object[] { SerializationConfig.Feature.FAIL_ON_EMPTY_BEANS,
				DeserializationConfig.Feature.USE_ANNOTATIONS, JsonParser.Feature.ALLOW_SINGLE_QUOTES,
				JsonGenerator.Feature.WRITE_NUMBERS_AS_STRINGS });
		beanFactory.setFeaturesToDisable(new Object[] { SerializationConfig.Feature.AUTO_DETECT_GETTERS,
				DeserializationConfig.Feature.AUTO_DETECT_FIELDS, JsonParser.Feature.AUTO_CLOSE_SOURCE,
		...
		assertTrue(objectMapper.getSerializationConfig().isEnabled(SerializationConfig.Feature.FAIL_ON_EMPTY_BEANS));
		assertTrue(objectMapper.getDeserializationConfig().isEnabled(DeserializationConfig.Feature.USE_ANNOTATIONS));
		assertTrue(objectMapper.getJsonFactory().isEnabled(JsonParser.Feature.ALLOW_SINGLE_QUOTES));
