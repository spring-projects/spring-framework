...
public class MessageBrokerConfigurationTests {
	...
	public void configureMessageConvertersDefault() {
		...
		assertThat(converters).extracting("class").containsOnlyOnce(
			StringMessageConverter.class,
			ByteArrayMessageConverter.class,
			KotlinSerializationJsonMessageConverter.class,
			MappingJackson2MessageConverter.class
		);

		MappingJackson2MessageConverter mappingJackson2MessageConverter = (MappingJackson2MessageConverter) converters.stream()
				.filter(messageConverter -> messageConverter instanceof MappingJackson2MessageConverter)
				.findFirst().orElseThrow();

		ContentTypeResolver resolver = mappingJackson2MessageConverter.getContentTypeResolver();
