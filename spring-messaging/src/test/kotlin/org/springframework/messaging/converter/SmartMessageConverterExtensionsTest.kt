...
class SmartMessageConverterExtensionsTest {

	@Test
	fun `SmartMessageConverter fromMessage inline extension`() {
		val converter = StringMessageConverter()
		val content = MessageBuilder.withPayload("ABC").setHeader(CONTENT_TYPE, TEXT_PLAIN).build()
		assertThat(converter.fromMessage<String>(content, null)).isEqualTo("ABC")
	}
}