...
[subs="verbatim,quotes"]
----
	uri = "https://example.com/hotels/{id}/bookings";
...
will perform a GET on `https://example.com/hotel%20list`. That means if the input URL
String is already encoded, it will be encoded twice -- i.e.
`https://example.com/hotel%20list` will become `https://example.com/hotel%2520list`. If
...
----
	...
	String s = template.postForObject("https://example.com/user", entity, String.class);
----
...
To use https://velocity.apache.org[Velocity] to create your email template(s), you will
...
https://www.ehcache.org/[EhCache], Gemfire cache,
