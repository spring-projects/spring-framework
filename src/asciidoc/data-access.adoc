...
[NOTE]
====
If you use `setBlobAsBinaryStream` or `setClobAsAsciiStream` or `setClobAsCharacterStream`
of the `DefaultLobHanlder`, you can specify a negative integer against the contentLength argument.
If you specify a negative integer, the `DefaultLobHandler` invoke a `PreparedStatement`'s
method without the content length that it added from the JDBC 4.0.
If will be specify a negative integer, please confirm the support status of the jdbc driver.
====
