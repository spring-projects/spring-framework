...
* A `WebAsyncTask<?>` can be returned when the application wants to produce the return
...
* A class which implements `CompletionStage<?>` (e.g., `CompletableFuture<?>`)
  can be returned when the application wants to produce the return value from a thread of
  its own choosing. (available on Java 8+)
