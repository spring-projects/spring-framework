...
https://bitworking.org/projects/URI-Templates/[proposed RFC] for URI Templates defines
how a URI is parameterized. For example, the URI Template
`https://www.example.com/users/{userId}` contains the variable __userId__. Assigning the
value __fred__ to the variable yields `https://www.example.com/users/fred`.
...
https://pivotal.io/security/cve-2015-5211[CVE-2015-5211].
...
==== Matrix Variables
The URI specification https://tools.ietf.org/html/rfc3986#section-3.3[RFC 3986] defines
...
more unique https://www.w3.org/DesignIssues/MatrixURIs.html["Matrix URIs"], originating
...
rendering with https://wiki.fasterxml.com/JacksonJsonViews[Jackson's Serialization Views].
...
https://www.w3.org/TR/eventsource/[Server-Sent Events].
...
https://blog.pivotal.io/pivotal/products/websocket-architecture-in-spring-4-0[the following blog post].
...
* Use a distinct URI for each resource, typically by using a different file extension in
  the URI. For example, the URI `https://www.example.com/users/fred.pdf` requests a PDF
  representation of the user fred, and `https://www.example.com/users/fred.xml` requests
  an XML representation.
* Use the same URI for the client to locate the resource, but set the `Accept` HTTP
  request header to list the https://en.wikipedia.org/wiki/Internet_media_type[media
  types] that it understands. For example, an HTTP request for
  `https://www.example.com/users/fred` with an `Accept` header set to `application/pdf`
  requests a PDF representation of the user fred, while
  `https://www.example.com/users/fred` with an `Accept` header set to `text/xml` requests
  an XML representation. This strategy is known as
  https://en.wikipedia.org/wiki/Content_negotiation[content negotiation].
...
a request for the following URL, `https://www.sf.net/home.view?siteLanguage=nl` will
...
implementation for use with https://jakarta.apache.org/commons/fileupload[__Commons
...
The https://projects.spring.io/spring-security/[Spring Security] project provides features
...
https://hdiv.org/[HDIV] is one such framework and integrates with Spring MVC.
...
An https://en.wikipedia.org/wiki/HTTP_ETag[ETag] (entity tag) is an HTTP response header
...
https://schema.spring.io/mvc/spring-mvc.xsd[Spring MVC XML schema] or use the code
