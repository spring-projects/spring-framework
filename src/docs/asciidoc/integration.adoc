...
whether the method has been already run for the given arguments. If it has been
run, the cached result is returned without having to run the actual method.
If the method has not been run, then it is run, and the result is cached and
returned to the user so that, the next time the method is invoked, the cached result is
returned. This way, expensive methods (whether CPU- or IO-bound) can be run only
once for a given set of parameters and the result reused without having to actually
run the method again. The caching logic is applied transparently without any
interference to the invoker.

IMPORTANT: This approach works only for methods that are guaranteed to return the same
output (result) for a given input (or arguments) no matter how many times it is run.
...
having to actually run the method. In its simplest form, the annotation declaration
...
the cached method was not actually run.
...
method is not cached (that is, the method is run every time no matter what values are in the cache
...
you can use the `@CachePut` annotation. That is, the method is always run and its
...
the method run by using the `beforeInvocation` attribute. The former provides the
...
performs no caching -- that is, it forces the cached methods to be run every time.
The following example shows how to do so:
