...
you can include `@Configuration` classes by using component scanning or defining them as normal
...
references a URL (for example, a path prefixed with `classpath:`, `file:`, or `http:`)
...
that references a URL (for example, a path prefixed with `classpath:`, `file:`, `http:`)
...
described in <<spring-mvc-test-server-htmlunit-mah-advanced-builder>>.
