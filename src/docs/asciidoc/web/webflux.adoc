...
- `ProblemDetails` -- representation for an RFC 7807 problem detail; a simple container
...
You can return `ProblemDetails` or `ErrorResponse` from any `@ExceptionHandler` or from
any `@RequestMapping` method to render an RFC 7807 response as follows:

- The `status` property of `ProblemDetails` determines the HTTP status.
- The `instance` property of `ProblemDetails` is set from the current URL path, if not
already set.
- For content negotiation, the Jackson `HttpMessageConverter` prefers
"application/problem+json" over "application/json" when rendering a `ProblemDetails`,
...
One, insert into the "properties" `Map` of `ProblemDetails`. When using the Jackson
library, the Spring Framework registers `ProblemDetailsJacksonMixin` that ensures this
...
You can also extend `ProblemDetails` to add dedicated non-standard properties.
The copy constructor in `ProblemDetails` allows a sub-class to make it easy to be created
from an existing `ProblemDetails`. This could be done centrally, e.g. from an
`@ControllerAdvice` such as `ResponseEntityExceptionHandler` that re-creates the
`ProblemDetails` of an exception into a subclass with the additional non-standard fields.
...
Message codes default to "problemDetails." + the fully qualified exception class name. Some
...
`ProblemDetails`, or a subclass of `ProblemDetails`.
