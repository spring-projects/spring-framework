...
* RDBMS objects, including `MappingSqlQuery`, `SqlUpdate` and `StoredProcedure`, require
  ...
  string, declare parameters, and compile the query. Once you do that, methods that have been run
...
top-level array's length indicates the number of batches run and the second level array's
