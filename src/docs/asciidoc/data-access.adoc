...
.Java
----
	RowMapper<Actor> actorRowMapper = (resultRest, rowNum) -> {
		Actor actor = new Actor();
		actor.setFirstName(resultRest.getString("first_name"));
		actor.setLastName(resultRest.getString("last_name"));
		return actor;
	};
	Actor actor = this.jdbcTemplate.queryForObject(
			"select first_name, last_name from t_actor where id = ?",
			actorRowMapper, 1212L);
----
[source,kotlin,indent=0,subs="verbatim,quotes",role="secondary"]
.Kotlin
----
